...
if __name__ == '__main__':
    ...
    expl_rate = [math.sqrt(2)]  # Different exploration rate for MCTS
    deals_list = [7]  # Different deals to test
    rollouts_list = [500] # Different rollouts to test

    for deals in deals_list:
        for rollouts in rollouts_list:
            for expl in expl_rate:
                start_time = time.time()
                print(f"\n=== Testing deals={deals}, rollouts={rollouts}, expl_rate={expl} ===")

                player_configs = [
                    {"type": "heuristic", "name": "H1"},
                    {"type": "heuristic", "name": "H2"},
                    {"type": "heuristic", "name": "H3"},
                    {"type": "mcts", "name": "MCTS_test", "deals": deals, "rollouts": rollouts, "expl_rate": expl, "scoring": "win_rate"}
                ]
